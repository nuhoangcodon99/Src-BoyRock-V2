...
import com.boyrock.models.task.SideTaskTemplate;
import com.boyrock.models.task.SideTaskTemplate1;
import com.boyrock.models.task.SubTaskMain;
import com.boyrock.models.task.TaskMain;
...
public class TaskService {
    ...
    private static com.boyrock.services.TaskService i;

    public static com.boyrock.services.TaskService gI() {
        if (i == null) {
            i = new com.boyrock.services.TaskService();
        }
        return i;
    }
    private Item item;
...
//        }
    ...
    if (player.playerTask != null && player.playerTask.taskMain != null) {
        ...
        // Xử lý trường hợp player.playerTask hoặc player.playerTask.taskMain là null (nếu cần thiết)
        // Ví dụ: trả về giá trị mặc định hoặc thông báo lỗi
        return false;
    }
}
    ...
    public void changeSideTask(Player player, byte level) {
      ...
      public void changeSideTask1(Player player, byte level) {
        if (player.playerTask.sideTask.leftTask > 0) {
            player.playerTask.sideTask.reset();
            SideTaskTemplate1 temp1 = Manager.SIDE_TASKS_TEMPLATE1.get(Util.nextInt(0, Manager.SIDE_TASKS_TEMPLATE1.size() - 1));
            player.playerTask.sideTask.template1 = temp1;
            player.playerTask.sideTask.maxCount = Util.nextInt(temp1.count[level][0], temp1.count[level][1]);
            player.playerTask.sideTask.leftTask--;
            player.playerTask.sideTask.level = level;
            player.playerTask.sideTask.receivedTime = System.currentTimeMillis();
            Service.getInstance().sendThongBao(player, "Bạn nhận được nhiệm vụ: " + player.playerTask.sideTask.getName1());
        } else {
            Service.getInstance().sendThongBao(player,
                    "Bạn đã nhận hết nhiệm vụ hôm nay. Hãy chờ tới ngày mai rồi nhận tiếp");
        }
...
public void removeSideTask1(Player player) {
        Service.getInstance().sendThongBao(player, "Bạn vừa hủy bỏ nhiệm vụ " + player.playerTask.sideTask.getName());
        player.playerTask.sideTask.reset1();
    }
                ...
                case ConstTask.EASY:                       
                   player.pointKarin+= 1;
                    ...
                    if (Util.isTrue(10, 100)){
                     Item item = ItemService.gI().createNewItem((short) 2030);
                                       
                                        InventoryServiceNew.gI().addItemBag(player, item);
                                        InventoryServiceNew.gI().sendItemBags(player);
                       Service.getInstance().sendThongBao(player, "Bạn nhận được 10tr vàng và 1 năng lượng kích hoạt và đá ma thuật");
                   
                    } else {                 
                                        
                    Service.getInstance().sendThongBao(player, "Bạn nhận được 50tr vàng và 1 năng lượng kích hoạt");
                    ...
                    Service.getInstance().sendThongBao(player, "Bạn nhận được 20tr vàng và 1 năng lượng kích hoạt");
                    break;
                case ConstTask.HARD:                       
              
                    player.pointKarin += 2;
                     player.inventory.gold += 30000000;
                     Service.getInstance().sendThongBao(player, "Bạn nhận được 30tr vàng và 2 năng lượng kích hoạt");
                    break;
                      case ConstTask.VERY_HARD:
                            player.pointKarin += 2;
                     player.inventory.gold += 40000000;
                     Service.getInstance().sendThongBao(player, "Bạn nhận được 40tr vàng và 2 năng lượng kích hoạt");
                    break;
                case ConstTask.HELL:
                    player.pointKarin += 3;
                     player.inventory.gold += 50000000;
                     Service.getInstance().sendThongBao(player, "Bạn nhận được 50tr vàng và 3 năng lượng kích hoạt");
                    break;
 ...
 public void paySideTask1(Player player) {
    if (player.playerTask.sideTask.template1 != null) {
        if (player.playerTask.sideTask.isDone()) {
            ...
            switch (player.playerTask.sideTask.level) {
                case ConstTask.EASY:                       
                   player.pointSukien += 1;
                    player.inventory.gold += 50000000;
                    Service.getInstance().sendThongBao(player, "Bạn nhận được 50tr vàng và 1 điểm lấp lánh");
                    break;
//                      case ConstTask.NORMAL:
//                           player.pointskh += 1;
//                    player.inventory.gold += 10000000;
//                    Service.getInstance().sendThongBao(player, "Bạn nhận được 10tr vàng và 1 điểm kích hoạt");
//                    break;
//                case ConstTask.HARD:                       
//              
//                    player.pointskh += 2;
//                     player.inventory.gold += 15000000;
//                     Service.getInstance().sendThongBao(player, "Bạn nhận được 15tr vàng và 2 điểm kích hoạt");
//                    break;
//                      case ConstTask.VERY_HARD:
//                            player.pointskh += 2;
//                     player.inventory.gold += 20000000;
//                     Service.getInstance().sendThongBao(player, "Bạn nhận được 20tr vàng và 2 điểm kích hoạt");
//                    break;
//                case ConstTask.HELL:
//                    player.pointskh += 3;
//                     player.inventory.gold += 30000000;
//                     Service.getInstance().sendThongBao(player, "Bạn nhận được 30tr vàng và 3 điểm kích hoạt");
//                    break;
            }
 ...
 public void checkDoneSideTaskKillMob1(Player player, Mob mob) {
        if (player.playerTask != null &&player.playerTask.sideTask.template1 != null) {
            if ( (player.playerTask.sideTask.template1.id == 86 && mob.tempId == ConstMob.BIRD)
             || (player.playerTask.sideTask.template1.id == 87 && mob.tempId == ConstMob.BEE)){
                player.playerTask.sideTask.count++;
                notifyProcessSideTask(player);
            }
        }
