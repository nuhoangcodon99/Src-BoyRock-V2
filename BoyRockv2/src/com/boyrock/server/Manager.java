...
import com.boyrock.jdbc.daos.ShopDAO;
...
import com.boyrock.models.matches.TOP;
...
import com.boyrock.models.player.Referee;



import com.boyrock.models.reward.ItemMobReward;
import com.boyrock.models.reward.ItemOptionMobReward;
import com.boyrock.models.reward.MobReward;
import com.boyrock.models.shop.Shop;
import com.boyrock.models.skill.NClass;
import com.boyrock.models.skill.Skill;
import com.boyrock.models.task.SideTaskTemplate;
import com.boyrock.models.task.ClanTaskTemplate;
...
public class Manager {
    ...
    public static final List<com.boyrock.models.map.Map> MAPS = new ArrayList<>();
    ...
    public static final List<SideTaskTemplate> SIDE_TASKS_TEMPLATE = new ArrayList<>();
     public static final List<SideTaskTemplate1> SIDE_TASKS_TEMPLATE1 = new ArrayList<>();
    ...
    private void initMap() {
        ...
        for (MapTemplate mapTemp : MAP_TEMPLATES) {
            ...
            com.boyrock.models.map.Map map = new com.boyrock.models.map.Map(mapTemp.id,
    ...
    private void loadDatabase() {
        ...
        try (Connection con = GirlkunDB.getConnection();) {
            ...
            ps = con.prepareStatement("select * from side_task_template1");
            rs = ps.executeQuery();
            while (rs.next()) {
                SideTaskTemplate1 sideTask = new SideTaskTemplate1();
                sideTask.id = rs.getInt("id");
                sideTask.name = rs.getString("name");
                String[] mc1 = rs.getString("max_count_lv1").split("-");
                ...
                SIDE_TASKS_TEMPLATE1.add(sideTask);
            }
            Logger.success("Load side task 1 thành công (" + SIDE_TASKS_TEMPLATE1.size() + ")\n");
            ...
            Logger.success("Load clan task thành công (" + CLAN_TASKS_TEMPLATE.size() + ")\n");
            // load item template
 ...
 for(int i = 1400;i < 1600; i++){
                if(ITEM_TEMPLATES.get(i).type == 5) CAITRANG.add(ItemService.gI().createNewItem((short) ITEM_TEMPLATES.get(i).id));
            }
              for(int i = 1000 ;i < 1500; i++){
                if(ITEM_TEMPLATES.get(i).type == 11) DEOLUNG.add(ItemService.gI().createNewItem((short) ITEM_TEMPLATES.get(i).id));
            }
            ...
            Logger.success("Load top SuKien thành công (" + topSuKien.size() + ")\n");
            topBoss = realTop(queryTopBoss, con);
            Logger.success("Load top boss thành công (" + topBoss.size() + ")\n");
            topNroNamec = realTop(queryTopNroNamec, con);
            Logger.success("Load top NroNamec thành công (" + topNroNamec.size() + ")\n");
