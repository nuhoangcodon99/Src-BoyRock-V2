...
public class PlayerDAO {
    ...
    public static boolean createNewPlayer(int userId, String name, byte gender, int hair) {
        try {
            ...
            int[] skillsArr = gender == 0 ? new int[] { 0, 1, 6, 9, 10, 20, 22, 24, 19 , 27, 28, 29,30}
                    : gender == 1 ? new int[] { 2, 3, 7, 11, 12, 17, 18, 26, 19, 27,28, 29,30 }
                            : new int[] { 4, 5, 8, 13, 14, 21, 23, 25, 19 , 27, 28, 29,30 } ;
            ...
            dataArray.clear();
 JSONArray VodaiBall = new JSONArray();
            for (int i = 1; i <= 7; i++) {
                VodaiBall.add(0);
                VodaiBall.add(0);
                VodaiBall.add(0);
                VodaiBall.add(blackBall.toJSONString());
                VodaiBall.clear();
            }
            String dataVodaiBall = dataArray.toString();
            dataArray.clear();
        ...
        if (player.iDMark.isLoadedAllDataPlayer()) {
            ...
            try {
                ...
                String query = "UPDATE player SET data_item_time_sieu_cap = ?, head = ?, have_tennis_space_ship = ?,"
                        ...
                        + " skills_shortcut = ?, pointPvp = ?, NguHanhSonPoint = ?, data_card = ?, bill_data = ?, data_goku_egg = ?, data_ngokhong_egg = ?,info_achievement = ?,pointBoss=?,data_thienthan_egg =?,data_hacam_egg=?,pointKarin = ?,pointNroNamec = ?,capChuyenSinh = ?,last_time_dd=?,count_open_box = ?,point_su_kien = ?,lvlThienSu =?,point_cau_ca=?, ismonkey = ?, isbroly =?,registeredVodai = ?,ischeckjoinvodai =? WHERE id = ?";
                ...
                GirlkunDB.executeUpdate(query,
                        ...
                        player.pointCauCa,
                           player.ismonkey,
                              player.isbroly,
                              player.registeredVodai,
                               player.ischeckjoinvodai,
                        player.id);
